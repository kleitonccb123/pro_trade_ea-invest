//+------------------------------------------------------------------+
//| Expert Advisor: PRICEPRO_M30                                     |
//| Variante: SWING BALANCE — Gráfico M30                            |
//| Estratégia: Swing com confirmação de reversão                     |
//| Lógica: RSI em modo retração (extremos sobrecomprado/sobrevendido)|
//|          Requer candle oposto ao anterior (pullback confirmado)   |
//|          EMA 34 como filtro de tendência intermediária            |
//|          TP maior que SL (R:R ≥ 1.5)                             |
//+------------------------------------------------------------------+
...
bool ValidarLicencaViaAPI(const string url)
{
   ...
   string ru = response; StringToUpper(ru);
   if(StringFind(ru, "AUTORIZAD") >= 0 || StringFind(ru, "AUTHORIZED") >= 0)
   {
      ProcessarRespostaLicenca(response);
      g_licenseValidated = true;
      Comment("✓ Licença: " + g_licenseAccountID);
      return true;
   }
   return false;
}
...
void ProcessarRespostaLicenca(const string response)
{
   ...
   for(int i = 0; i < len; i++)
   {
      if(arr[i] >= '0' && arr[i] <= '9')
      {
         int j = i; while(j < len && arr[j] >= '0' && arr[j] <= '9') j++;
         if(j - i >= 8) { g_licenseExpire = (datetime)StringToInteger(StringSubstr(response, i, j - i)); break; }
         i = j;
      }
   }
}
...
//--- M30: modo retração (RSI em extremos) — captura pullbacks  
input bool   RSI_ImpulsoMode       = false;
input bool   UsarForcaCandle       = true;
//--- Confirmação: exige que o candle atual reverta (candle oposto ao pullback)
input bool   UsarCandleReversal    = true;
...
//--- Média Móvel — EMA 34 (equilíbrio entre velocidade e ruído no M30)
input bool   AtivarMediaMovel      = true;
input int    PeriodoMediaMovel     = 34;
input ENUM_MA_METHOD TipoMedia     = MODE_EMA;

//--- RSI — período 9 rápido, níveis 35/65 (não tão extremos = mais consistente no M30)
input int    PeriodoRSI            = 9;
input double RSI_Sobrecomprado     = 65.0;
input double RSI_Sobrevendido      = 35.0;

//--- Candle mínimo 45% no M30 (um pouco menos exigente que H1)
input double CorpoCandleMinimo     = 45.0;

//--- Volume M30 — fator intermediário
input double VolumeMinimo          = 0.60;

//--- Range M30
input int    PeriodoRange          = 9;
input double MultiplicadorRange    = 1.20;

//--- TP com R:R 1.5 (ex: TP=18 SL=12 → 1.5)
input double TP_USD                = 18.0;
input double SL_USD                = 12.0;
...
//--- Geral
input ulong  NumeroMagico          = 20260002;
input ENUM_TIMEFRAMES TempoGrafico = PERIOD_M30;

//--- Breakeven M30: 600 pontos
input int    BreakevenActivatePoints = 600;
...
#define PP_BG     "M30_BG"
#define PP_TITLE  "M30_TITLE"
#define PP_PROFIT "M30_PROFIT"
#define PP_VALUE  "M30_VALUE"
...
ulong  g_lastPanelUpdate = 0, g_lastHasPosTick = 0;
...
double MoneyToPriceDistance(double money, double volume)
{
   ...
   return (tv>0.0&&ts>0.0) ? (money*ts)/(tv*volume) : 0.0;
}
double CalculateLotSize()
{
   double lot=0.01;
   if(UsarLoteFixo) lot=LoteFixo;
   else if(UsarLotePorRisco) lot=(AccountInfoDouble(ACCOUNT_BALANCE)*(PercentualRisco/100.0))/1000.0;
   ...
   if(mn>0.0) lot=MathMax(lot,mn); if(mx>0.0) lot=MathMin(lot,mx);
   if(st>0.0) lot=MathRound(lot/st)*st;
...
double GetCandleBodyPercent(int shift=1)
{
   double r=iHigh(_Symbol,PERIOD_CURRENT,shift)-iLow(_Symbol,PERIOD_CURRENT,shift);
   return (r<=0.0)?0.0:(MathAbs(iClose(_Symbol,PERIOD_CURRENT,shift)-iOpen(_Symbol,PERIOD_CURRENT,shift))/r)*100.0;
}
double GetVolumeRatio()
{
   double v1=(double)iVolume(_Symbol,PERIOD_CURRENT,1), v2=(double)iVolume(_Symbol,PERIOD_CURRENT,2), v3=(double)iVolume(_Symbol,PERIOD_CURRENT,3);
   double avg=(v1+v2+v3)/3.0; return (avg<=0.0)?0.0:v1/avg;
}
double GetRangeRatio()
{
   ...
   for(int i=1;i<=PeriodoRange;i++) { double r=iHigh(_Symbol,PERIOD_CURRENT,i)-iLow(_Symbol,PERIOD_CURRENT,i); if(r>0.0){sum+=r;cnt++;} }
   if(cnt==0) return 0.0; double avg=sum/cnt; return (avg<=0.0)?0.0:r1/avg;
}
double GetDailyProfit()
{
   ulong now=GetTickCount64(); if(now-g_lastStatsUpdateTick<1000) return g_cachedDailyProfit;
   ...
   double p=0.0; int deals=HistoryDealsTotal();
   for(int i=0;i<deals;i++) { ulong t=HistoryDealGetTicket(i); if(t==0) continue; p+=HistoryDealGetDouble(t,DEAL_PROFIT)+HistoryDealGetDouble(t,DEAL_SWAP)+HistoryDealGetDouble(t,DEAL_COMMISSION); }
...
{
   ulong now=GetTickCount64(); if(now-g_lastHasPosTick<50) return g_cachedHasPos;
   g_lastHasPosTick=now; g_cachedHasPos=false;
   for(int i=0;i<PositionsTotal();i++) { ulong t=PositionGetTicket(i); if(t==0) continue; if(PositionGetString(POSITION_SYMBOL)==_Symbol&&(ulong)PositionGetInteger(POSITION_MAGIC)==NumeroMagico){g_cachedHasPos=true;break;} }
...
//--- CheckTrend M30: preço vs EMA (simples, sem confirmação de candle)
...
//--- Variação M30: verifica se candle anterior é de reversão (oposto à tendência = pullback confirmado)
bool CheckCandleReversal(int direction)
{
   if(!UsarCandleReversal) return true;
   // candle 2 = candle que formou o pullback (vai contra a tendência)
   double open2  = iOpen(_Symbol, PERIOD_CURRENT, 2);
   double close2 = iClose(_Symbol, PERIOD_CURRENT, 2);
   // candle 1 = candle de reversão (fecha na direção da tendência)
   double open1  = iOpen(_Symbol, PERIOD_CURRENT, 1);
   double close1 = iClose(_Symbol, PERIOD_CURRENT, 1);
   if(direction == DIR_BUY)
      return (close2 < open2) && (close1 > open1); // pullback baixista + reversão altista
   if(direction == DIR_SELL)
      return (close2 > open2) && (close1 < open1); // pullback altista + reversão baixista
   return false;
}
...
void OpenPosition(int direction, double volume)
{
   ...
   else if(ModoDebug) Print("M30 ORDEM: ",(direction==DIR_BUY?"COMPRA":"VENDA")," Vol:",volume," P:",price," SL:",sl," TP:",tp);
}
...
void UIPanel_Create()
{
   ObjectCreate(0,PP_BG,OBJ_RECTANGLE_LABEL,0,0,0); ObjectSetInteger(0,PP_BG,OBJPROP_CORNER,CORNER_LEFT_UPPER);
   ...
   ObjectSetInteger(0,PP_BG,OBJPROP_BGCOLOR,C_BG_DARK); ObjectSetInteger(0,PP_BG,OBJPROP_BORDER_TYPE,BORDER_FLAT);
   ObjectSetInteger(0,PP_BG,OBJPROP_COLOR,C'30,50,80'); ObjectSetInteger(0,PP_BG,OBJPROP_WIDTH,2);
   ObjectCreate(0,PP_TITLE,OBJ_LABEL,0,0,0); ObjectSetInteger(0,PP_TITLE,OBJPROP_CORNER,CORNER_LEFT_UPPER);
   ...
   ObjectSetString(0,PP_TITLE,OBJPROP_FONT,"Arial Bold"); ObjectSetString(0,PP_TITLE,OBJPROP_TEXT,"PRICEPRO M30 – Swing Balance");
   ObjectCreate(0,PP_PROFIT,OBJ_LABEL,0,0,0); ObjectSetInteger(0,PP_PROFIT,OBJPROP_CORNER,CORNER_LEFT_UPPER);
   ...
   ObjectCreate(0,PP_VALUE,OBJ_LABEL,0,0,0); ObjectSetInteger(0,PP_VALUE,OBJPROP_CORNER,CORNER_LEFT_UPPER);
   ObjectSetInteger(0,PP_VALUE,OBJPROP_XDISTANCE,130); ObjectSetInteger(0,PP_VALUE,OBJPROP_YDISTANCE,55);
...
void ManagePositionStops()
{
   ...
   for(int i=0;i<PositionsTotal();i++)
   {
      ...
      if(!hsBE)
      {
         double diff=(type==POSITION_TYPE_BUY)?(bid-openP)/_Point:(openP-ask)/_Point;
         if(diff>=(double)BreakevenActivatePoints)
         {
            double nSL=NormalizeDouble(openP,(int)_Digits);
            bool mv=(curSL<=0.0)||(type==POSITION_TYPE_BUY&&nSL>curSL+MinMovePoints*_Point)||(type==POSITION_TYPE_SELL&&nSL<curSL-MinMovePoints*_Point);
            if(mv){bool mod=trade.PositionModify(ticket,nSL,curTP);if(ModoDebug)Print("BE M30 t=",ticket," SL=",nSL," ok=",mod);AddBreakevenTicket(ticket);int idx=FindBreakevenIndex(ticket);if(idx!=-1)g_breakevenLastModifiedBar[idx]=curBar;}
         }
      }
      else if(AtivarTrailingPorCandle)
      {
         int idx=FindBreakevenIndex(ticket);
         datetime prevBar=iTime(_Symbol,PERIOD_CURRENT,1); if(prevBar==0||(idx!=-1&&g_breakevenLastModifiedBar[idx]==prevBar)) continue;
         double nSL=NormalizeDouble((type==POSITION_TYPE_BUY)?iLow(_Symbol,PERIOD_CURRENT,1):iHigh(_Symbol,PERIOD_CURRENT,1),(int)_Digits);
         bool mv=(curSL<=0.0)?((type==POSITION_TYPE_BUY&&nSL>=openP+MinMovePoints*_Point)||(type==POSITION_TYPE_SELL&&nSL<=openP-MinMovePoints*_Point)):((type==POSITION_TYPE_BUY&&nSL>curSL+MinMovePoints*_Point)||(type==POSITION_TYPE_SELL&&nSL<curSL-MinMovePoints*_Point));
         if(mv&&MathAbs(nSL-curSL)>=_Point*MinMovePoints){bool mod=trade.PositionModify(ticket,nSL,curTP);if(ModoDebug)Print("Trail M30 t=",ticket," SL=",nSL," ok=",mod);if(idx==-1)AddBreakevenTicket(ticket);idx=FindBreakevenIndex(ticket);if(idx!=-1)g_breakevenLastModifiedBar[idx]=prevBar;}
      }
   }
}
...
int OnInit()
{
   ...
   if(Period()!=TempoGrafico) Print("Aviso: Este EA é otimizado para M30.");
   trade.SetExpertMagicNumber(NumeroMagico);
   ...
   UIPanel_Create();
   Print("PRICEPRO M30 - Swing Balance iniciado | Magic=",NumeroMagico," | EMA=",PeriodoMediaMovel," | RSI=",PeriodoRSI);
...
void OnTick()
{
   ...
   if(IsNewBar())
   {
      ...
      bool okReversal= CheckCandleReversal(dir);
      ...
      if(ModoDebug)
      {
         ...
         Print("M30 [",d,"] RSI=",DoubleToString(rsiVal,1)," ok=",okRSI,
               " Reversal=",okReversal," Candle=",DoubleToString(GetCandleBodyPercent(),1),"% ok=",okCandle,
               " Vol=",DoubleToString(GetVolumeRatio(),2)," ok=",okVol,
               " Range=",DoubleToString(GetRangeRatio(),2)," ok=",okRange);
      }

      if(okRSI && okReversal && okCandle && okVol && okRange)
      {
