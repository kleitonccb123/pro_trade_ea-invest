...
- Redis (when REDIS_URL is set): fast RPUSH/BLPOP signalling — low latency wakeup
- MongoDB: durable task storage + status tracking (always used regardless of Redis)
"""
import asyncio
import logging
...
# Redis queue key used for fast wakeup signalling
_REDIS_QUEUE_KEY = "task_queue:pending"
...
async def _get_redis():
    ...
    try:
        ...
        return _aioredis.from_url(_cfg.redis_url, decode_responses=True)
    except Exception:
        return None
...
class TaskQueue:
    ...
    async def enqueue_task(self, task_type: TaskType, bot_id: str, user_id: str) -> str:
        ...
        Always persists to MongoDB for durability.
        Also pushes the task_id to Redis (when available) for immediate wakeup.
        """
        db = get_db()
        ...
        # Signal via Redis for low-latency wakeup
        redis = await _get_redis()
        if redis:
            try:
                await redis.rpush(_REDIS_QUEUE_KEY, task_id)
            except Exception as exc:
                logger.debug("[task_queue] Redis signal failed (non-fatal): %s", exc)
            finally:
                await redis.aclose()
    ...
    async def _process_queue_loop(self):
        ...
        Uses Redis BLPOP for instant wakeup when Redis is available,
        otherwise falls back to polling MongoDB every 5 seconds.
        """
        while self.is_running:
            try:
                redis = await _get_redis()
                if redis:
                    # Block up to 5 seconds waiting for a task signal
                    try:
                        result = await redis.blpop(_REDIS_QUEUE_KEY, timeout=5)
                    except Exception:
                        result = None
                    finally:
                        await redis.aclose()
                ...
                # Only sleep when Redis is unavailable (Redis BLPOP already blocked)
                if not redis:
